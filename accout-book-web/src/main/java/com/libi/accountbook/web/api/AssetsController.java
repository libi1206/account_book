...
public class AssetsController extends BaseController implements BaseAttrController<AssetsDto> {
    ...
    public ResponseDto create(AssetsDto assetsDto) {
        assetsService.insertAssets(assetsDto,getLoginUser().getId());
        return new ResponseDto(0, "创建成功", null);
    }

    @Override
    @PostMapping("/update")
    public ResponseDto update(AssetsDto assetsDto) {
        assetsService.updateAssets(assetsDto);
        return new ResponseDto(0, "修改成功", assetsService.getAssetsById(assetsDto.getId()));
    }
